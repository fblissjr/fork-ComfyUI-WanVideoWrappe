    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "offload_img_emb": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Offload img_emb to offload_device"},
                ),
                "offload_txt_emb": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Offload time_emb to offload_device"},
                ),
            },
        }
...
class WanVideoLoraBlockEdit:
    ...
    def select(self, **kwargs):
        selected_blocks = {
            k: v for k, v in kwargs.items() if v is True and isinstance(v, bool)
        }
...
class WanVideoModelLoader:
    ...
    def loadmodel(
        ...
        log.info(
            f"Model type: {model_type}, num_heads: {num_heads}, num_layers: {num_layers}"
        )
        ...
        if not "torchao" in quantization:
            ...
            if vram_management_args is not None:
                from .diffsynth.vram_management import (
                ...
                total_params_in_model = sum(
                    p.numel() for p in patcher.model.diffusion_model.parameters()
                )
                log.info(
                    f"Total number of parameters in the loaded model: {total_params_in_model}"
                )
            ...
            if compile_args is not None:
                ...
                if compile_args["compile_transformer_blocks"]:
                    for i, block in enumerate(patcher.model.diffusion_model.blocks):
                        patcher.model.diffusion_model.blocks[i] = torch.compile(
                            block,
        ...
        patcher.model["auto_cpu_offload"] = (
            True if vram_management_args is not None else False
        )
...
class WanVideoTextEncode:
    ...
    def process(self, t5, positive_prompt, negative_prompt, force_offload=True):
        ...
        with torch.autocast(
            device_type=mm.get_autocast_device(device), dtype=dtype, enabled=True
        ):
...
class WanVideoImageClipEncode:
    ...
    def process(
        ...
        resized_image = common_upscale(
            image.movedim(-1, 1), w, h, "lanczos", "disabled"
        )
        ...
        if noise_aug_strength > 0.0:
            resized_image = add_noise_to_reference_video(
                resized_image, ratio=noise_aug_strength
            )
        ...
        concatenated = torch.concat(
            [resized_image.to(device), zero_frames, resized_image.to(device)], dim=1
...
class WanVideoSampler:
    ...
    def process(
        ...
        assert not (context_options and teacache_args), (
            "Context options cannot currently be used together with teacache."
        )
        patcher = model
        model = model.model
        ...
        if "sparge" in transformer.attention_mode:
            ...
            for idx, block in enumerate(transformer.blocks):
                ...
                block.self_attn.inner_attention = SparseAttentionMeansim(
                    l1=0.06, pv_l1=0.065
                )
            if transformer.attention_mode == "spargeattn":
                try:
                    ...
                    raise ValueError(
                        "No saved parameters found for sparse attention, tuning is required first"
                    )
                load_sparse_attention_state_dict(
                    transformer, saved_state_dict, verbose=True
                )
            ...
            for i, t in enumerate(tqdm(timesteps)):
                ...
                if context_options is not None:
                    counter = torch.zeros_like(
                        latent_model_input[0], device=intermediate_device
                    )
                    ...
                    for c in context_queue:
                        ...
                        if min(c) > 0:
                            ramp_up = torch.linspace(
                                0, 1, context_overlap, device=noise_pred.device
                            )
                        ...
                        if max(c) < latent_video_length - 1:
                            ramp_down = torch.linspace(
                                1, 0, context_overlap, device=noise_pred.device
                            )
                    ...
                    noise_pred_cond = transformer(
                        ...
                        if "image_embeds" in image_embeds
                        else [],
        ...
        if teacache_args is not None:
            log.info(
                f"TeaCache skipped: {transformer.teacache_skipped_cond_steps} cond steps, {transformer.teacache_skipped_uncond_steps} uncond steps"
            )
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "min_val": (
                    "FLOAT",
                    {"default": -0.15, "min": -1.0, "max": 0.0, "step": 0.0001},
                ),
                "max_val": (
                    "FLOAT",
                    {"default": 0.15, "min": 0.0, "max": 1.0, "step": 0.0001},
                ),
                "r_bias": (
                    "FLOAT",
